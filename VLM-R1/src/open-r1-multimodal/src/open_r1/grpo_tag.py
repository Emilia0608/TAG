...
def mcq_reward(content, sol, **kwargs):
    ...
    if sol_match:
        ...
        # 2) ### The final answer is: 뒤의 텍스트 파싱
        final_match = re.search(r'###\s*The final answer is:\s*(.+)', sol)
        if final_match:
            ground_truth = final_match.group(1).strip()
        else:
            # 3) 그래도 못 찾으면 전체 fallback
...
def table_keywords_check(content, keywords):
    ...
    for content, sol, accu_reward_method in zip(contents, solution, kwargs.get("accu_reward_method")):
        ...
        if accu_reward_method == "mcq":
            ...
            if keywords is not None:
                tag_r = table_keywords_check(content, keywords)
                reward += tag_r * 0.1
